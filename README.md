# StockAnalysis - 台股資訊平台

一個基於 React 和 SCSS 開發的現代化股票資訊應用程式，專為台灣證券市場設計。
提供即時股價、K線圖表、繪圖工具和財經新聞等功能。

## 功能特色

- **即時股票資訊** - 整合多個 API 來源，提供即時股價資料
- **股票觀察清單** - 可新增、刪除和管理個人關注的股票
- **互動式 K 線圖表** - 使用 Chart.js 提供專業級股票圖表和繪圖工具
- **財經新聞** - 顯示最新財經和科技新聞（利用 GNews API）
- **響應式設計** - 支援桌面、平板和手機等多種裝置

## 技術架構

### 前端技術
- **React** - 使用 Hooks 進行狀態管理
- **SCSS** - 模組化樣式管理
- **Chart.js** - K線圖表渲染和繪圖工具

### API 整合
- **富果 API** - 獲取即時股票資料和 K 線數據
- **Yahoo Finance API** - 備用股票資料來源
- **GNews API** - 財經、科技新聞

### 主要功能

1. **觀察清單管理**
   - 點擊「新增」按鈕添加股票代號（例如：2330）
   - 點擊股票卡片右上角的選單可刪除股票
   - 支援左右滑動瀏覽股票列表

2. **股票資訊查看**
   - 點擊股票卡片查看詳細資訊
   - 顯示當日行情數據（開盤、收盤、最高、最低等）
   - 整合 K 線圖表和繪圖工具

3. **進階圖表**
   - 點擊「切換至全螢幕圖表」進入圖表頁面
   - 提供 K 線圖表、直線繪圖、矩形繪圖、趨勢線等工具
   - 支援多時間框架（日K、30分K、5分K）

4. **新聞瀏覽**
   - 瀏覽最新財經和科技新聞
   - 點擊新聞卡片開啟原文連結

### 支援的股票代號格式

- 四位數字：`2330`（自動轉換為 2330.TW）
- 完整格式：`2330.TW`
